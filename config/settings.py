...
This module is written to work inside the reusable ``gen_ai_core_lib`` package.
It loads environment variables from a ``.env`` file in the current working
directory (where your application runs), which is the most typical pattern
for libraries used across different projects.
"""
import os
...
class Settings:
    ...
    # Project Information
    PROJECT_NAME: str = os.getenv("PROJECT_NAME", "GenAI core app")
    PROJECT_DESCRIPTION: str = os.getenv("PROJECT_DESCRIPTION", "A GenAI application")
    ...
    # Note: HR assistant configuration (model, agent pool, vector store, memory) is now in hr_agent_config.yaml
    ...
    LANGCHAIN_PROJECT: str = os.getenv("LANGCHAIN_PROJECT", "gen-ai-core")
